...
class Conversation:
    ...
    def get_prompt(self) -> str:
        ...
        if self.sep_style == SeparatorStyle.ADD_COLON_SINGLE:
            ...
            seps = [self.sep, self.sep, self.sep2]
            ret = self.system + seps[0]
            for i, (role, message) in enumerate(self.messages):
                try:
                    if message:
...
register_conv_template(
    Conversation(
        name="tool_llama",
        system="A chat between a curious user and an artificial intelligence assistant who can use external vision tools to solve the user's question. "
        "The assistant gives tools calling processes or final answer to the human's question.",
        roles=("Human", "Assistant"),
        messages=(),
        offset=0,
        sep_style=SeparatorStyle.ADD_COLON_TWO,
...
register_conv_template(
    Conversation(
        ...
        system="A chat between a curious user and an artificial intelligence assistant who can use external vision tools to solve the user's question. "
        "The assistant gives tools calling processes or final answer to the human's question.",
        roles=("Human", "Assistant", "Assistant_rejected"),
        messages=(),
        offset=0,
...
if __name__ == "__main__":
    ...
    conv = get_conv_template("tool_llama")
